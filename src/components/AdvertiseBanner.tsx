...
export default function AdvertiseBanner() {
  ...
  return (
    ...
    >
      {/* Floating container */}
      <div
        className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand to-brand-dark text-white shadow-2xl ring-1 ring-black/5 translate-y-0 md:-translate-y-1 will-change-transform"
        ...
        <div className="relative h-full">
          {slides.map((s, i) => (
            ...
            >
              ...
              <div className="relative px-6 sm:px-10 py-16 sm:py-20 md:py-24 pb-24 sm:pb-28 md:pb-32 text-center">
                <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tight mb-3">{s.title}</h2>
                {s.subtitle && (
                  <p className="opacity-90 max-w-2xl mx-auto">{s.subtitle}</p>
                )}
                {s.cta && (
                  <div className="mt-6 flex justify-center">
                    <Link href={s.cta.href} className="btn btn-accent">
                      {s.cta.label}
                    </Link>
                  </div>
                )}
          ...
          <div className="invisible px-6 sm:px-10 py-16 sm:py-20 md:py-24 pb-24 sm:pb-28 md:pb-32">
